...
class MenuMain(Menu):
    def __init__(self, goodChain):
        ...
        Menu.__init__(self, goodChain, f"Welcome to GoodChain.\nGoodChain currently has {goodChain.last_block.block_id+1} {"block" if goodChain.last_block.block_id == 0 else "blocks"} with {goodChain.count_tx()} total transactions.", items, functions)
